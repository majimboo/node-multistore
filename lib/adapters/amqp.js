...
AmqpAdapter.prototype.schema = function (name, schema, options) {
  return Schema.add('amqp', name, schema, options);
};

AmqpAdapter.prototype.create = function (table, data, config) {
  ...
  return this.client.then(function (ch) {
    if (exchange) {
      ...
      ch.publish(exchange, key, new Buffer(JSON.stringify(data)));
    } else {
      ch.checkQueue(key);
