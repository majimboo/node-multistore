...
msgpack_pack_inline_func(_ext)(msgpack_pack_user x, size_t l, int8_t type)
{
  ...
  if(l == 1) {
		unsigned char buf[2] = { 0xd4, t };
		msgpack_pack_append_buffer(x, buf, 2);
  } else if(l == 2) {
		unsigned char buf[2] = { 0xd5, t };
		msgpack_pack_append_buffer(x, buf, 2);
  } else if(l == 4) {
		unsigned char buf[2] = { 0xd6, t };
		msgpack_pack_append_buffer(x, buf, 2);
  } else if(l == 8) {
		unsigned char buf[2] = { 0xd7, t };
		msgpack_pack_append_buffer(x, buf, 2);
  } else if(l == 16) {
		unsigned char buf[2] = { 0xd8, t };
		msgpack_pack_append_buffer(x, buf, 2);
  } else if(l < 256) {
#ifdef __cplusplus
	 	unsigned char buf[3] = { 0xc7, static_cast<unsigned char>(l), t };
#else
	 	unsigned char buf[3] = { 0xc7, (unsigned char)l, t };
#endif
	 	msgpack_pack_append_buffer(x, buf, 3);
  } else if(l < 65536) {
		unsigned char buf[4];
    buf[0] = 0xc8; _msgpack_store16(&buf[1], (uint16_t)l); buf[3] = t;
		msgpack_pack_append_buffer(x, buf, 4);
  } else {
		unsigned char buf[6];
    buf[0] = 0xc9; _msgpack_store32(&buf[1], (uint32_t)l); buf[5] = t;
