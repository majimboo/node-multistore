...
#define push_variable_value(func, base, pos, len) \
  ...
  goto _push
#define push_variable_value_with_type(func, base, pos, ext_type, len) \
  if(msgpack_unpack_callback(func)(user, \
    (const char*)base, (const char*)pos, len, ext_type, &obj) < 0) { goto _failed; } \
  goto _push
  ...
  do {
    ...
    case CS_HEADER:
      ...
      SWITCH_RANGE(0xc0, 0xdf)  // Variable
        ...
        case 0xd7:  // fixext 8
        case 0xd8:  // fixext 16
          again_fixed_trail(NEXT_CS(p), 1 << ((((unsigned int)*p) & 0x0f)-4));
        ...
        case 0xc8:  // ext 16
        case 0xc9:  // ext 32
          again_fixed_trail(NEXT_CS(p), (1 << ((((unsigned int)*p) & 0x0f)-7) ));
    ...
    default:
      ...
      case CS_EXT_8:
        again_fixed_trail(ACS_EXT_VALUE, *((uint8_t *)n)+1); // +1 because of byte containing type
      case CS_EXT_16:
        again_fixed_trail(ACS_EXT_VALUE, _msgpack_load16(uint16_t,n)+1); // +1 because of byte containing type
      case CS_EXT_32:
        again_fixed_trail(ACS_EXT_VALUE, _msgpack_load32(uint32_t,n)+1); // +1 because of byte containing type
      ...
      case CS_FIXEXT_8:
      case CS_FIXEXT_16:
        push_variable_value_with_type(_ext, data, n+1, *((int8_t *)n), trail);

      case ACS_RAW_VALUE:
      _raw_zero:
        push_variable_value(_raw, data, n, trail);
      case ACS_EXT_VALUE:
        push_variable_value_with_type(_ext, data, n+1, *((int8_t *)n), trail-1); // -1 because type already included in the push, so 1 byte has been decoded
